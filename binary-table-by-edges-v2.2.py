...
parser = argparse.ArgumentParser(prog='Binary table by edges',
                                 usage='$python binary-table-by-edges-v2.2.py -t [tree_file] -a [alignment_file]')
...
# Read the FASTA alignment file and record values for all alignment positions
...
# For all-identical positions, all edges will be assigned 0's
    ...
    # If not, compute substitutions
        ...
        for item in edges_all:
            ...
            if pos_values[node1] != pos_values[node2]:
                mutated_edges.append('1')  # Edge with substitution

            else:
                mutated_edges.append('0')  # Edge without substitution
